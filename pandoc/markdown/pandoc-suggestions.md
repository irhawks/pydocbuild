...
## 违反直觉的图表应该通通去除掉[12-29-2016 22:44:38 CST]

许多时候我们想添加一些富文本或者表格格式的东西，这些东西本身具有一定的结构化表示。这个时候，直接放在markdown里面不是很好处理。这个时候，我们可以换一种方式处理：仍然在markdown当中采用自由格式，但是在markdown当中添加新的标记，这样的标记可以让markdown自由地渲染内容。这个时候，自己想到的是利用`<<<`这样的一种标记，来表示两段之间的内容需要markdown作处理，它可以看成是`~~~`的一种变体。因为markdown并不怎么理解`~~~`里面的内容，而侧向于keep verbatim。`<<<`表示的语义是go to parse it。

所以，在保持高亮的同时，我们也更进一步展示了结构。比如Definition List可以这样展示：

```
下面是一个Definition List

<<< .def style=asparaenum
概念1, 解释性的内容
概念2, 解释性的内容
<<<
```
...
还有就是如果是图片格式，可以使用TikZ之类的标记

<<< .tikz
a -> b -> c
<<<

这里，我们只要把内容放在`<<<`与`<<<`之间，markdown就会自动地知道，我们并不是想要里面的源代码，而是希望把源代码处理掉，得到源代码处理掉之后的结果。而如果是`~~~`开头，则通常表示我们默认只是想要源代码而已。这样的话，还统一了图片等处理方式。我们可以认为，`<<<`括起来的内容，表示的是need to be processed。

还有，自己觉得，位图可以采用Base64编码的格式，或者采用缩略图的方法。展示出来一个大略的结果。还有，展示位图的时候，解析成ASCII，就像是`mplayer -vo caca MovieName.avi`一样可以在终端当中直接以终端的彩色播放电影一样。明天继续吧。或许在Jupyter里面可以非常容易地就做到这一点吧。

类似地，像Theorem之类的环境，也应该放在`<<<`这样的环境里面。表示的是需要转换的内容。

## pandoc的一种新的标记类型[12-31-2016 11:25:29 CST]

自己发现有些软件借用了一行开头的`#`号。因为默认情况下若干个`#`开头，然后跟一个空格表示的是层次标题。但是其实这样开头的行也可以复用它们的标记。比如使用`##+ `这样的内容作为开头，表示一些特殊的Block。这在Rmarkdown里面好像比较多见。
